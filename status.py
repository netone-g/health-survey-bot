    ...
    if event.get("source") == "aws.events":
        ...
        for org in organizations:
            ...
            for admin_email in org["admins"]:
                try:
                    result = send_message_to_webex(admin_email, "  \n".join(messages))
        ...
        # helpmessage
        help_message = 'Enter "check" to get a list of users who have not reported their safety.  \nEnter "list" to see all the answers'

        # Webex Message decode
        mess_ret = get_message_from_webex(message_id)
        ...
        if original_message == "help":
            results.append(send_message_to_webex(email, help_message))
            ...
            for org in target_orgs:
                ...
                results.append(send_message_to_webex(email, "## {}  \n  \n".format(org["name"]) + message))
            ...
            for org in target_orgs:
                ...
                if len(anpiret['yet_users']) == 0:
                    ...
                    messages.append("No unanswered respondents")
                else:
...
def get_message_from_webex(id: str):
